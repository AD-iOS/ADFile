...
@property (nonatomic, strong) NSArray *updateLogs;
...
- (void)viewDidLoad {
    ...
    self.title = @"更新日志";
    [self setupUpdateLogs];
    [self.tableView registerClass:[UITableViewCell class] forCellReuseIdentifier:@"UpdateCell"];
}

- (void)setupUpdateLogs {
    self.updateLogs = @[
        @"1.6-objc (Objective-C) - 使用Objective-C重新写了一遍目前有两个版本分别是由swift写的版本和现在的Objective-C版本(注:此日志Objective-C版本专属)",
